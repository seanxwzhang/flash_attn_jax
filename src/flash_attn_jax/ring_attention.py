...
def ring_fwd(q,k,v, axis_name, axis_size, mha_fwd, softmax_scale=None, is_causal=False, similarity=flash_api.softmax, deg=4):
    [n,l,h,d] = q.shape
        ...
        if is_causal:
            ...
            o2, lse2 = jax.lax.switch(cmp,
                                    [
                                        ...
                                        lambda: mha_fwd(q,k,v, softmax_scale=softmax_scale, is_causal=True, window_size=(-1,-1),similarity=similarity, deg=deg),
                                        lambda: mha_fwd(q,k,v, softmax_scale=softmax_scale, is_causal=False, window_size=(-1,-1),similarity=similarity, deg=deg),
                                    ])
        else:
            o2, lse2 = mha_fwd(q,k,v, softmax_scale=softmax_scale, is_causal=False, window_size=(-1,-1),similarity=similarity, deg=deg)
...
def ring_bwd(do,q,k,v,o,lse, axis_name, axis_size, mha_bwd, softmax_scale=None, is_causal=False, similarity=flash_api.softmax, deg=4):
    [n,l,h,d] = q.shape
    [n,lk,hk,d] = k.shape
    if softmax_scale is None:
        softmax_scale = 1/math.sqrt(d)
    ...
    def f(acc, _):
        ...
        def causal():
            return mha_bwd(do,q,k2,v2,o,lse, softmax_scale=softmax_scale, is_causal=True, window_size=(-1,-1), similarity=similarity, deg=deg)
        def non_causal():
            return mha_bwd(do,q,k2,v2,o,lse, softmax_scale=softmax_scale, is_causal=False, window_size=(-1,-1), similarity=similarity, deg=deg)
